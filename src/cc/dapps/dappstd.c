...
char ASSETCHAINS_SYMBOL[SQUISHY_ASSETCHAIN_MAXLEN];
char IPADDRESS[100];
...
int32_t games_sendrawtransaction(char *rawtx)
{
    ...
    if ( (retstr= squishy_issuemethod(USERPASS,(char *)"sendrawtransaction",params,GAMES_PORT)) != 0 )
    {
...
int32_t games_setplayerdata(struct games_state *rs,char *gametxidstr)
{
    ...
    if ( 0 )
    {
        ...
        sprintf(cmd,"./squishy-cli -ac_name=%s cclib gameinfo 17 \\\"[%%22%s%%22]\\\" > %s",ASSETCHAINS_SYMBOL,gametxidstr,fname);
        if ( system(cmd) != 0 )
    ...
    else
    {
        ...
        filestr = squishy_issuemethod(USERPASS,(char *)"cclib",params,GAMES_PORT);
    }
    if ( filestr != 0 )
    {
...
int main(int argc, char **argv)
{
    ...
    GAMES_PORT = squishy_userpass(userpass,ASSETCHAINS_SYMBOL);
    if ( IPADDRESS[0] == 0 )
    ...
    else
    {
        ...
        if ( GAMES_PORT == 0 )
        {
            printf("you must copy %s.conf from ~/.squishy/%s/%s.conf (or equivalent location) to current dir\n",ASSETCHAINS_SYMBOL,ASSETCHAINS_SYMBOL,ASSETCHAINS_SYMBOL);
            return(-1);
        }
